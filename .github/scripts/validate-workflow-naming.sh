...
done < <(find "$workflows_dir" -maxdepth 1 -type f \( -name 'common-*.yml' -o -name 'common-*.yaml' \) -print0 | sort -z)
...
done < <(find "$workflows_dir" -maxdepth 1 -type f \( -name '*.yml' -o -name '*.yaml' \) ! -name 'common-*.yml' ! -name 'common-*.yaml' -print0 | sort -z)
