 ...
 * - Version does not contain SNAPSHOT (when publishing)
 *
    ...
    fun register(project: Project) {
        ...
        project.plugins.withId("maven-publish") {
            project.tasks.register("validatePublications") { task ->
                ...
                task.description = "Validates that Maven publications meet Central Portal requirements"
                task.doLast {
                    val publishing = project.extensions.getByType(PublishingExtension::class.java)
                    val publications = publishing.publications.withType(MavenPublication::class.java)

                    if (publications.isEmpty()) {
                        project.logger.warn("validatePublications: no MavenPublication found in ${project.path}")
                        return@doLast
                    }
                    ...
                    for (pub in publications) {
                        ...
                        // Check artifact classifiers for sources and javadoc
                    ...
                    if (errors.isNotEmpty()) {
                        val message =
                            buildString {
                                ...
                                appendLine()
                                appendLine("Fix these before publishing. See https://central.sonatype.org/publish/requirements/")
                            }
                        throw org.gradle.api.GradleException(message)
                    }

                    project.logger.lifecycle(
                        "validatePublications: ${publications.size} publication(s) in ${project.path} passed Maven Central checks",
                    )
                }
            }
