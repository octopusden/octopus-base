...
| **GitHub Packages publish fails** after Central succeeded | yes | no | no | no | No, and the state is the one above *plus* an absent package. Recover the release with `recover-release.sh`; the version can then be published to GitHub Packages only by re-running that step, since the version cannot be re-published there either. |
