...
# command substitution inside `[ ]` discards the exit status - so a sort that cannot do -V
# returned nothing, the "went backwards" branch was skipped, and post-processing proceeded on
# a version older than the last one. Silently. Both values are digits and dots by now.
