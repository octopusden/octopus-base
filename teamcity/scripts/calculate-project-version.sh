...
# guard's newest release. Newest first, in git's version order.
#
...
if [ -f "$file" ]; then
  ...
  if [ "${IS_DEFAULT_BRANCH-}" = true ]; then
    ...
    while IFS= read -r tag; do
      ...
      released_major=${BASH_REMATCH[1]}; released_minor=${BASH_REMATCH[2]}
      if (( 10#$major < 10#$released_major ||
            (10#$major == 10#$released_major && 10#$minor < 10#$released_minor) )); then
...
# Newest tag on the line. The regex is what admits a tag: v2.0.6.1 and v2.0.64-rc1 are not on
# line 2.0.
patch=
while IFS= read -r tag; do
